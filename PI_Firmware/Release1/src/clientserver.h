...
conn_t* connect_tcp(const char* server_IP, const char* server_Port) {
	...
	conn_t* server_conn = (conn_t*)malloc(sizeof(conn_t));
	...
	server_conn->sockfd = socket(result->ai_family, result->ai_socktype, result->ai_protocol);

	status = connect(server_conn->sockfd, result->ai_addr, result->ai_addrlen);
